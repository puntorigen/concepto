  ...
  class concepto {
    ...
    */
    ...
    prepareCommandsAutocomplete() {
      ...
      return _asyncToGenerator(function* () {
        ...
        _this2.x_console.outT({
          message: "Adding x_commands autocomplete definitions!",
          color: "cyan"
        });

        var ac = {};

        for (var cmd in _this2.x_commands) {
          //if command has autocomplete
          if (cmd != 'meta' && _this2.x_commands[cmd].autocomplete) {
            //for each autocomplete key
            for (var key in _this2.x_commands[cmd].autocomplete) {
              ...
              var tmpItem = _this2.x_commands[cmd].autocomplete[key];
              tmpItem.id = key;
              ac[key] = tmpItem;
            }
          }
        ...
        yield _this2.generateAutoComplete(ac);
      })();
    }
    ...
    generateAutoComplete(auto) {
      ...
      return _asyncToGenerator(function* () {
        ...
        Object.keys(auto).forEach(key => {
          ...
          auto2[key] = _objectSpread2(_objectSpread2({}, baseObj), auto[key]);

          if (!auto2[key].text) {
            auto2[key].text = key;
          }
        });
        auto = _objectSpread2({}, auto2);
        /* add support for remapping attributes.'posibleChildren' key existance */

        Object.keys(auto).forEach(tag => {
          var cleanedTag = tag.replaceAll('*-', '').replaceAll('-*', '').replaceAll(':*', ':'); // add 'type' to auto[tag] if 'icons' contain 'idea'=view, 'help'=event, 'penguin'=script, 'desktop_new'=command
          ...
          Object.keys(auto[tag].attributes).forEach(attr_ => {
            ...
            var attr = auto[tag].attributes[attr_]; // also add the attribute to the auto object if it contains {icon:list} within its keyname

            var attrIcons = extractIcons(attr_).icons;

            if (attrIcons.length == 1 && attrIcons[0] == 'list') {
              if (attr.type.length > 0) {
                for (var attrType of attr.type.split(',')) {
                  ...
                  if (attrType != 'object' && attrType != 'array') {
                    ...
                    var typeNameForNewChildren = cleanedTag + '-' + attr__ + '-' + attrType; // add typeNameForNewChildren to attr childrenTypes
                    ...
                    var newTagName = cleanedTag + '-' + attr__ + '-' + attrType;
                    ...
                    if (!auto[newTagName]) {
                      auto[newTagName] = {
                        ...
                        icons: [],
                        level: [],
                        hint: "Value of ".concat(attrType, " for attribute ").concat(attr__, " of ").concat(tag),
                        childrenTypes: ['none'],
                        attributes: {}
                      };
                    } else {
            ...
            if (attr_.indexOf('{icon:list}') != -1) {
              ...
              var _typeNameForNewChildren = cleanedTag + '-' + type__ + '-' + attr__; // add typeNameForNewChildren to attr childrenTypes


              if (auto[tag].childrenTypes && auto[tag].childrenTypes.length > 0) {
                auto[tag].childrenTypes.push(_typeNameForNewChildren);
              } else {
                auto[tag].childrenTypes = [_typeNameForNewChildren];
              } //gen unique name for attrType (newTagName)


              var _newTagName = _typeNameForNewChildren; //

              if (!auto[_newTagName]) {
                auto[_newTagName] = {
                  text: attr.text ? attr.text : attr__,
                  type: _typeNameForNewChildren,
                  parentsAll: [cleanedTag],
                  parents: [cleanedTag],
                  //@TODO this should be parentTypes, or parentsAll (but IDE lacks support for it yet)
                ...
                if (attr.parentsAll && Array.isArray(attr.parentsAll)) {
                  auto[_newTagName].parentsAll = Array.concat(attr.parentsAll, auto[_newTagName].parentsAll);
                } // if attr.text also exists within auto[*].text and childrenTypes here is empty, 
                // then copy childrenTypes and parentsAll from it


                var targetAC = Object.keys(auto).filter(key => {
                  var item = auto[key];

                  if (item && auto[_newTagName].type.indexOf(item.type) != -1 && item.childrenTypes && item.childrenTypes.length > 0 && item.text == auto[_newTagName].text) {
                    return true;
                  }
                });
                var targetNode = targetAC.length > 0 ? auto[targetAC[0]] : null; //console.log('targetNode',targetNode);

                var weNode = auto[_newTagName];

                if (targetNode && weNode.childrenTypes.length == 0) {
                  auto[_newTagName].childrenTypes = [...targetNode.childrenTypes];

                  if (targetNode.parentsAll.length > 0 && weNode.parentsAll.length == 0) {
                    auto[_newTagName].parentsAll = [...targetNode.parentsAll];
                  }
                }
              } else {
                // if it already existed, add to parents array if it was not there already
                if (!auto[_newTagName].parents.includes(cleanedTag)) auto[_newTagName].parents.push(cleanedTag);
              } //
            ...
            Object.keys(auto[tag].events).forEach(event => {
              if (!auto[event]) {
                auto[event] = {
                  ...
                  attributes: auto[tag].events[event].params ? _this3.params2attr(auto[tag].events[event].params) : auto[tag].events[event].attributes ? auto[tag].events[event].attributes : {}
                };
              } else {
                // if it already existed, add to parents array if it was not there already
        ...
        for (var key in tags) {
          yield _this3.addAutocompleteDefinition(tags[key]);
        }
      })();
    }
    ...
    cleanAutocompleteFiles() {
      ...
      return _asyncToGenerator(function* () {
        ...
        try {
          for (var file of yield fs.readdir(_this4.autocomplete.path)) {
            yield fs.unlink(path.join(_this4.autocomplete.path, file));
          }
        } catch (xx) {}
      })();
    }
    ...
    * @return 	{string}										- HTML template
    ...
    generateAutocompleteFiles() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var tmp_directory = path.dirname(path.resolve(_this5.x_flags.dsl));
        ...
        if (!_this5.autocomplete) {
          ...
          _this5.autocomplete.path = autocomplete_path;
        ...
        var meta = _this5.x_commands.meta;
        delete meta.hint;
        ...
        try {
          ...
          try {
            yield copy(iconsPath, _this5.autocomplete.path, {
              overwrite: true
            });
          } catch (ercp) {}
        } catch (eeeec) {
        ...
        */
        ...
        var prepareInherited = record => {
          ...
          if (record.extends_ && record.extends_ != '' && _this5.autocomplete.refs[record.extends_]) {
            ...
            var parentAttribs = Object.keys(_this5.autocomplete.refs[record.extends_].attributes);
            ...
            var parentEvents = Object.keys(_this5.autocomplete.refs[record.extends_].events);
        ...
        var jsonKeys = Object.keys(_this5.autocomplete.json);
        var autoJson = {
          nodes: []
        };

        if (jsonKeys.length > 0) {
          var file = path.join(_this5.autocomplete.path, 'autocomplete.json'); // generate the html for the hints

          for (var key of jsonKeys) {
            var record = _objectSpread2(_objectSpread2({}, _this5.autocomplete.json[key]), _this5.autocomplete.refs[key]);

            if (record.hint) {
              record.hint = yield _this5.autocompleteContentTemplate(prepareInherited(record), {
                icon: _icon => {
              ...
              record.hint = "<BASE href=\"file://".concat(_this5.autocomplete.path, "/\">\n") + record.hint; //record.hint = `<body bgcolor="white">`+record.hint+`</body>`;
            }

            _this5.autocomplete.json[key].hint = record.hint;
            record = _this5.autocomplete.json[key]; //this may not be needed anymore
          ...
          yield _this5.writeFile(file, json);
        }
      })();
    }
    /**
    ...
    addAutocompleteDefinition() {
      ...
      return _asyncToGenerator(function* () {
        ...
        */
        ...
        //add to def
        var hash = yield _this6.dsl_parser.hash(_arguments);
        if (id == '') id = text;
        _this6.autocomplete.records[hash] = {
          keys: [],
          bestKey: '',
          text,
          icons,
        ...
        _this6.autocomplete.refs[id] = {
          attributes,
          events,
          extends_
        };
        return _this6.autocomplete.records[hash];
      })();
    } // **********************************
    ...
    addCommands(command_func) {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (command_func && typeof command_func === 'function') {
          var t = yield command_func(_this8);

          if (typeof t === 'object') {
            // if there was a meta, add new info as name child
            if (_this8.x_commands && _this8.x_commands.meta && t.meta && t.meta.name) {
              if (!_this8.x_commands.meta.other) _this8.x_commands.meta.other = {};
              _this8.x_commands.meta.other[t.meta.name] = t.meta;
              delete t.meta;
            }

            _this8.x_commands = _objectSpread2(_objectSpread2({}, _this8.x_commands), t);
          } else {
          ...
          // if there was a meta, add new info as name child
          if (_this8.x_commands && _this8.x_commands.meta && command_func.meta && command_func.meta.name) {
            if (!_this8.x_commands.meta.other) _this8.x_commands.meta.other = {};
            _this8.x_commands.meta.other[command_func.meta.name] = command_func.meta;
            delete command_func.meta;
          }

          _this8.x_commands = _objectSpread2(_objectSpread2({}, _this8.x_commands), command_func);
    ...
    */
    ...
    cacheCommands() {
      ...
      return _asyncToGenerator(function* () {
        ...
        for (var x in _this9.x_commands) {
          if (_this9.x_commands[x].x_watch) {
            var watched = _this9.x_commands[x].x_watch.split(',');
            ...
            for (var xi in watched) {
              ...
              if (key.includes('x_state.')) {
                ...
                try {
                  watched_vars[x][key.trim()] = safe('this.' + key.trim(), _this9);
                } catch (doesnt_exist) {
          ...
          if (x == 'meta') {
            x_cmds_hashes[x] = yield _this9.dsl_parser.hash(JSON.stringify(_this9.x_commands[x]));
          } else {
            x_cmds_hashes[x] = yield _this9.dsl_parser.hash(_this9.x_commands[x].func.toString());
          }
        } //console.log('obj_to_hash',x_cmds_hashes);


        var commands_hash = yield _this9.dsl_parser.hash(x_cmds_hashes);
        var commands_cache = yield _this9.cache.getItem('commands_hash');
        var commands_cached_hashes = yield _this9.cache.getItem('commands_hashes');
        var changed_x_cmds = [];
        var watched_vars_hash_now = yield _this9.dsl_parser.hash(watched_vars);
        var watched_vars_hash_cache = yield _this9.cache.getItem('watched_vars_cache');
        var watched_vars_cache = yield _this9.cache.getItem('watched_vars'); //console.log('PABLO debug: watched_vars_now',{current:watched_vars,cache:watched_vars_cache});
        ...
        if (commands_cache != commands_hash || watched_vars_hash_now != watched_vars_hash_cache) {
          if (typeof commands_cached_hashes === 'object') {
            ...
            if (changed_x_cmds.includes('meta')) {
              ...
              yield _this9.cache.clear();
              yield _this9.cleanAutocompleteFiles();
            } else {
              if (changed_x_cmds.length > 0) _this9.x_console.outT({
                prefix: 'cache,yellow',
              ...
              var meta_cache = yield _this9.cache.getItem('meta_cache');
              var amount_cleaned = 0;

              if (meta_cache && typeof meta_cache === 'object' && Object.keys(meta_cache).length > 0) {
                for (var _x2 in meta_cache) {
                  if (_this9.array_intersect(meta_cache[_x2].x_ids.split(','), changed_x_cmds).length > 0) {
                    ...
                    yield _this9.cache.removeItem(meta_cache[_x2].cachekey);
                    yield _this9.cache.removeItem(meta_cache[_x2].cachekey + '_x_state');
                    amount_cleaned += 1;
                  }
                }
              ...
              yield _this9.cleanAutocompleteFiles(); //erase autocomplete files
          ...
          yield _this9.cache.setItem('watched_vars', watched_vars);
          yield _this9.cache.setItem('watched_vars_hash_now', watched_vars_hash_now);
          yield _this9.cache.setItem('commands_hash', commands_hash);
          yield _this9.cache.setItem('commands_hashes', x_cmds_hashes);
        }
      })();
    }
    /**
    ...
    */
    ...
    findCommand() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var resp = _objectSpread2(_objectSpread2({}, _this10.reply_template()), {
          id: 'not_found',
        ...
        if (typeof node.icons === 'undefined') {
          if (show_debug) _this10.debug('error: findCommand was given a blank node!');
          return resp;
        }

        var cache_key = node.id + node.date_modified.toString(); //+node.nodes_raw.toString();

        var t_cache = yield _this10.cache.getItem(cache_key);

        if (t_cache && (Array.isArray(t_cache) && t_cache.length > 0 || t_cache.data)) {
          // node.id in this.x_memory_cache.findCommand
          if (show_debug) _this10.debug("using memory_cache for findCommand for node ID ".concat(node.id));
          return t_cache;
        } else {
          if (show_debug) _this10.debug("findCommand for node ID ".concat(node.id));
          ...
          delete _this10.x_commands['constructor']; //4-jun-21 @todo double check this; maybe we should use a cloned var
          // iterate through commands

          for (var key in _this10.x_commands) {
            ...
            var command_requires = _objectSpread2(_objectSpread2({}, command_defaults), _this10.x_commands[key]);
            ...
            if (command_requires['x_not_icons'] != '' && allTrue(matched, keys)) {
              ...
              if (node.icons.length > 0 && command_requires['x_not_icons'] != '' && ['*'].includes(command_requires['x_not_icons'])) {
                ...
                matched.x_not_icons = _this10.array_intersect(command_requires['x_not_icons'].split(','), node.icons).length > 0 ? false : true;
              }
              ...
              for (var test of com_reqs) {
                ...
                if (test.indexOf('.') != -1) {
                  ...
                  (function () {
                    ...
                    var keys = _this10.dsl_parser.findVariables({
                      text: test,
                      symbol: '[',
                      symbol_closing: ']'
                    }).split('+');
                    ...
                    if (_this10.array_intersect(has_keys, keys).length != keys.length) {
                      matched.x_not_empty = false;
                    }
                  })();
                } else {
            ...
            if (command_requires['x_or_hasparent'] != '' && allTrue(matched, keys)) {
              ...
              matched.x_or_hasparent = yield _this10.hasParentID(node.id, command_requires['x_or_hasparent']);
            ...
            if (command_requires['x_all_hasparent'] != '' && allTrue(matched, keys)) {
              ...
              matched.x_all_hasparent = yield _this10.hasParentID(node.id, command_requires['x_all_hasparent'], true);
            ...
            if (command_requires['x_or_isparent'] != '' && allTrue(matched, keys)) {
              ...
              for (var _key7 of command_requires['x_or_isparent'].split(',')) {
                is_direct = yield _this10.isExactParentID(node.id, _key7);
            ...
            if (typeof command_requires['x_text_pattern'] === 'string' && command_requires['x_text_pattern'] != '' && allTrue(matched, keys)) {
              ...
              matched.x_text_pattern = _this10.x_match(node.text.trim(), command_requires['x_text_pattern']);
            ...
            if (Array.isArray(command_requires['x_text_pattern']) == true && command_requires['x_text_pattern'].length > 0 && allTrue(matched, keys)) {
              ...
              for (var xtp of command_requires['x_text_pattern']) {
                _test = _this10.x_match(text_trim, xtp);
                if (_test == true) break;
              }
          ...
          if (show_debug) _this10.debug_time({
          ...
          if (show_debug) _this10.debug_timeEnd({
            id: "sorting by priority"
          }); // reply
          ...
          if (Array.isArray(resp) && resp.length > 0 || resp.data) {
            ...
            yield _this10.cache.setItem(cache_key, resp);
    ...
    */
    findValidCommand() {
      ...
      return _asyncToGenerator(function* () {
        var {
          node = _this12.throwIfMissing('node'),
          object = false,
          x_command_shared_state = {},
          show_debug = true
        } = _arguments3.length > 0 && _arguments3[0] !== undefined ? _arguments3[0] : {};
        if (!_this12.x_flags.init_ok) throw new Error('error! the first called method must be init()!');

        if (node.id in _this12.x_memory_cache.findValidCommand) {
          ...
          return _this12.x_memory_cache.findValidCommand[node.id];
        } else {
          if (show_debug) _this12.debug({
          ...
          var commands_ = yield _this12.findCommand({
            node,
            justone: false,
          ...
          if (commands_.length == 0) {
            ...
            try {
              //if (object==true) {
              var test = yield _this12.x_commands[reply.x_id].func(node, x_command_shared_state);
              reply.exec = test; //}
              ...
              yield _this12.showLineError(test_err); // @TODO emit('internal_error','findValidCommand')
            ...
            for (var qm_index in commands_) {
              ...
              if (_this12.x_commands[qm.x_id]) {
                try {
                  var _test2 = yield _this12.x_commands[qm.x_id].func(node, x_command_shared_state);
                  ...
                  yield _this12.showLineError(test_err1);
                  reply.error = true;
                  reply.valid = false;
          ...
          _this12.x_memory_cache.findValidCommand[node.id] = reply;
          return reply;
        }
      })();
    } // ****************************
    ...
    */
    ...
    process() {
      ...
      return _asyncToGenerator(function* () {
        yield _this13.cacheCommands();
        if (!_this13.x_flags.init_ok) throw new Error('error! the first called method must be init()!');
        ...
        var x_dsl_nodes = yield _this13.dsl_parser.getNodes({
          level: 2,
          nodes_raw: true,
          hash_content: true
        }); // add abort support
        ...
        _this13.debug('calling prepareCommandsAutocomplete');

        _this13.debug_time({
          id: 'prepareAC'
        });

        yield _this13.prepareCommandsAutocomplete();

        _this13.debug_timeEnd({
          id: 'prepareAC'
        });

        _this13.debug('calling onPrepare');

        _this13.debug_time({
          id: 'onPrepare'
        });

        yield _this13.onPrepare();

        _this13.debug_timeEnd({
          id: 'onPrepare'
        });

        if (!_this13.x_config.debug && !_this13.x_config.silent) {
          _this13.progress_multi = {};
          _this13.multibar = _this13.x_console.progress({
          ...
          _this13.progress_multi['_total_'] = _this13.multibar.create(x_dsl_nodes.length - 1, {
            total_: 'x',
            screen: 'initializing..'
          });
        }
        ...
        var counter_ = 0;
        var meta_cache = {};
        var meta_cache_ = yield _this13.cache.getItem('meta_cache');
        ...
        for (var level2 of x_dsl_nodes) {
          ...
          //break;
          if (!_this13.x_config.debug && !_this13.x_config.silent) {
            if (_this13.progress_last) _this13.progress_last.raw().stop();
            _this13.progress_multi[level2.text] = _this13.multibar.create(level2.nodes_raw.length - 1, {
              total_: '',
              screen: 'initializing..',
              error: false
            });

            _this13.progress_multi['_total_'].update(counter_, {
              total_: 'x',
              screen: level2.text,
              error: false
            });

            _this13.progress_last = _this13.progress_multi[level2.text];
            _this13.progress_last_screen = level2.text;
          }

          if (_this13.abort && !_this13.x_config.debug && !_this13.x_config.silent) {
            _this13.progress_multi[level2.text].raw().stop();

            break; //abort processing if request
          } else if (_this13.abort) {
            break; //abort processing if request
          } //cache: check if current node has any children that were modified since last time


          var main = yield _this13.cache.getItem(level2.hash_content); // remove await when in production (use Promise.all after loop then)

          if (!main) {
            var before_state = JSON.parse(JSON.stringify(_this13.x_state));
            main = yield _this13.process_main(level2, {});
            var state_to_save = obj_diff(before_state, _this13.x_state); //console.log('state_to_save',{ state_to_save });
            ...
            if (main.error && main.error == true) {
              ...
              yield _this13.cache.setItem(level2.hash_content, main);
              yield _this13.cache.setItem(level2.hash_content + '_x_state', state_to_save);
            } //console.log('metido al cache:'+level2.id,main);

          } else {
            var cached_state = yield _this13.cache.getItem(level2.hash_content + '_x_state');
            _this13.x_state = deep.all([_this13.x_state, cached_state]);

            if (main.error && main.error == true) {
              yield _this13.cache.removeItem(level2.hash_content); //reprocess the removed failed cached node.

              var _before_state = JSON.parse(JSON.stringify(_this13.x_state));

              main = yield _this13.process_main(level2, {});

              var _state_to_save = obj_diff(_before_state, _this13.x_state); //console.log('state_to_save2',{ state_to_save });
              ...
              if (main.error && main.error == true) ; else {
                ...
                yield _this13.cache.setItem(level2.hash_content, main);
                yield _this13.cache.setItem(level2.hash_content + '_x_state', _state_to_save);
              }
          ...
          if (!_this13.x_config.debug && !_this13.x_config.silent) {
            if (main.error && main.error == true) {
              ...
              _this13.progress_multi[level2.text].total(level2.nodes_raw.length - 1);

              _this13.progress_multi[level2.text].update(level2.nodes_raw.length - 1, {
                screen: level2.text,
                sub: '',
                total_: ''
              });
        ...
        if (!_this13.x_config.debug && !_this13.x_config.silent) {
          _this13.progress_multi['_total_'].raw().stop(); //.remove();


          _this13.multibar.stop();
        ...
        yield _this13.generateAutocompleteFiles(); // if there was no error

        if (!were_errors && !_this13.abort) {
          // request creation of files
          yield _this13.onCreateFiles(resp.nodes);

          _this13.x_console.title({
            title: "Interpreter ".concat(_this13.x_config.class.toUpperCase(), " ENDED. Full Compilation took: ").concat(_this13.secsPassed_()),
            color: 'green'
          });

          _this13.debug_table('Amount of Time Per Command');
        } else if (_this13.abort) {
          _this13.x_console.title({
            title: "Interpreter ".concat(_this13.x_config.class.toUpperCase(), " ABORTED. Partial Compilation took: ").concat(_this13.secsPassed_()),
            color: 'green'
          });
        } else {
          // errors occurred
          _this13.x_console.title({
            title: "Interpreter ".concat(_this13.x_config.class.toUpperCase(), " ENDED with ERRORS.\nPlease check your console history.\nCompilation took: ").concat(_this13.secsPassed_()),
        ...
        yield _this13.cache.setItem('last_compile_date', new Date()); //add meta cache to cache

        yield _this13.cache.setItem('meta_cache', meta_cache);
        yield _this13.onEnd();
        return resp;
      })();
      ...
      return _asyncToGenerator(function* () {
        ...
        if (resp.hasChildren == true && resp.valid == true && nodei.getNodes) {
          ...
          if (!_this14.x_config.debug && !_this14.x_config.silent) {
            _this14.progress_last.total(sub_nodes.length - 1); //this.progress_multi['_total_'].raw().updateETA();
          ...
          for (var sublevel of sub_nodes) {
            ...
            var real = yield _this14.dsl_parser.getNode({
              id: sublevel.id,
              nodes_raw: true,
              recurse: false
            }); //this.debug('sub_process before findValidCommand node id:'+real.id);

            var real2 = yield _this14.findValidCommand({
              node: real,
              object: false,
            ...
            if (!_this14.x_config.debug && !_this14.x_config.silent) {
              _this14.progress_last.update(xx, {
            ...
            if (real2 && real2.exec && real2.exec.valid == true) {
              ...
              resp = yield _this14.sub_process(resp, sublevel, new_state);
              resp.code += real2.exec.close;
            } else if (real2.error == true) {
              if (!_this14.x_config.debug && !_this14.x_config.silent) {
                _this14.progress_last.total(xx);
              ...
              yield _this14.onErrors(["Error executing func for x_command:".concat(real2.x_id, " for node id ").concat(real.id, ", text: ").concat(real.text, " ")]);
    ...
    showLineError(error) {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (error_info.file && (yield _this15.exists(error_info.file)) == true) {
    ...
    process_main(node, custom_state) {
      ...
      return _asyncToGenerator(function* () {
        var resp = {
          state: custom_state,
          id: node.id,
          name: yield _this16.onDefineNodeName(node),
          file: yield _this16.onDefineFilename(node),
          init: '',
          title: yield _this16.onDefineTitle(node),
        ...
        if (_this16.x_config.debug && !_this16.x_config.silent) {
        ...
        var test = yield _this16.findValidCommand({
          node: node,
          object: false,
        ...
        if (test && test.exec && test.exec.valid == true) {
          ...
          if (typeof node.getNodes === 'function') {
            resp = yield _this16.sub_process(resp, node, copy_state);
          }
          ...
          yield _this16.onErrors(["Error executing func for x_command:".concat(test.x_id, " for node id ").concat(node.id, ", text: ").concat(node.text, " ")]);

          if (!_this16.x_config.debug && !_this16.x_config.silent) {
            _this16.progress_last.total(2);

            _this16.progress_last.update(1, {
              screen: _this16.progress_last_screen + '\n',
              error: true
            });
          } // improved error logging


          yield _this16.showLineError(test.catch);
          resp.valid = false, resp.hasChildren = false, resp.error = true;
        } else {
          _this16.x_console.outT({
          ...
          yield _this16.onErrors(["No method found for given node id ".concat(node.id, ", text: ").concat(node.text, " ")]);
          resp.valid = false, resp.hasChildren = false, resp.error = true;
        } // closing level2 'on' calls


        resp = yield _this16.onAfterProcess(resp);
        resp = yield _this16.onCompleteCodeTemplate(resp); //
    ...
    */
    ...
    _appFolders(keys) {
      ...
      return _asyncToGenerator(function* () {
        var compile_folder = _arguments4.length > 1 && _arguments4[1] !== undefined ? _arguments4[1] : _this17.x_state.central_config.apptitle;
        var output_dir = _arguments4.length > 2 ? _arguments4[2] : undefined;

        var fs = require('fs').promises;

        _this17.debug('_appFolders');

        var path = require('path');

        var dsl_folder = path.dirname(path.resolve(_this17.x_flags.dsl)) + path.sep;
        if (output_dir) dsl_folder = output_dir;
        var resp = {
          base: dsl_folder,
          src: dsl_folder + (compile_folder ? compile_folder : _this17.x_state.central_config.apptitle) + path.sep
        };
    ...
    */
    ...
    hasBrotherID() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments5.length > 0 && _arguments5[0] !== undefined ? _arguments5[0] : _this18.throwIfMissing('id-brotherID');
        var x_id = _arguments5.length > 1 && _arguments5[1] !== undefined ? _arguments5[1] : _this18.throwIfMissing('x_id');

        // @TODO test it after having 'real' commands on some parser 3-ago-20
        if (id + x_id in _this18.x_memory_cache.hasBrotherID) {
          return _this18.x_memory_cache.hasBrotherID[id + x_id];
        } else {
          ...
          for (var q of brother_ids) {
            var node = yield _this18.dsl_parser.getNode({
              id: q,
              recurse: false
            });
          ...
          _this18.x_memory_cache.hasBrotherID[id + x_id] = resp;
          return resp;
        }
      })();
    }
    ...
    */
    ...
    hasBrotherBefore() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments6.length > 0 && _arguments6[0] !== undefined ? _arguments6[0] : _this19.throwIfMissing('id-brotherBefore');

        if (id in _this19.x_memory_cache.hasBrotherBefore) {
          return _this19.x_memory_cache.hasBrotherBefore[id];
        } else {
          var brother_ids = yield _this19.dsl_parser.getBrotherNodesIDs({
            id,
            before: true,
            after: false
          }).split(',');
          _this19.x_memory_cache.hasBrotherBefore[id] = brother_ids.includes(id);
          return _this19.x_memory_cache.hasBrotherBefore[id];
        }
      })();
    }
    ...
    */
    ...
    hasBrotherNext() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments7.length > 0 && _arguments7[0] !== undefined ? _arguments7[0] : _this20.throwIfMissing('id - BrotherNext');

        if (id in _this20.x_memory_cache.hasBrotherNext) {
          return _this20.x_memory_cache.hasBrotherNext[id];
        } else {
          var brother_ids = yield _this20.dsl_parser.getBrotherNodesIDs({
            id,
            before: false,
            after: true
          }).split(',');
          _this20.x_memory_cache.hasBrotherNext[id] = brother_ids.includes(id);
          return _this20.x_memory_cache.hasBrotherNext[id];
        }
      })();
    }
    ...
    */
    ...
    isExactParentID() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments8.length > 0 && _arguments8[0] !== undefined ? _arguments8[0] : _this21.throwIfMissing('id-ExactParent');
        var x_id = _arguments8.length > 1 && _arguments8[1] !== undefined ? _arguments8[1] : _this21.throwIfMissing('x_id');

        // @TODO test it after having 'real' commands on some parser 4-ago-20
        if (id + x_id in _this21.x_memory_cache.isExactParentID) {
          return _this21.x_memory_cache.isExactParentID[id + x_id];
        } else {
          var parent_node = yield _this21.dsl_parser.getParentNode({
            id
          });
          var parent_command = yield _this21.findValidCommand({
            node: parent_node,
            show_debug: false,
            object: true
          });
          ...
          if (parent_command && parent_command.x_id == x_id) {
            _this21.x_memory_cache.isExactParentID[id + x_id] = true;
            return true;
          }

          _this21.x_memory_cache.isExactParentID[id + x_id] = false;
          return false;
        }
      })();
    }
    ...
    */
    ...
    hasParentID() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments9.length > 0 && _arguments9[0] !== undefined ? _arguments9[0] : _this22.throwIfMissing('id-ParentID');
        var x_id = _arguments9.length > 1 && _arguments9[1] !== undefined ? _arguments9[1] : _this22.throwIfMissing('x_id');
        ...
        for (var parent_id of parents) {
          ...
          var parentCommand = yield _this22.findValidCommand({
            node,
            show_debug: false,
            object: true
          });
          ...
          if (onlyTrueIfAll == false && x_ids.includes(parentCommand.x_id)) {
            ...
            if (_this22.array_intersect(tested_parents_x_ids, x_ids).length == x_ids.length) {
              return true;
            }
          }
    ...
    */
    ...
    getParentIDs() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments10.length > 0 && _arguments10[0] !== undefined ? _arguments10[0] : _this23.throwIfMissing('id-ParentIDs');
        var array = _arguments10.length > 1 && _arguments10[1] !== undefined ? _arguments10[1] : false;
        // @TODO test it after having 'real' commands on some parser 4-ago-20
        var parents = yield _this23.dsl_parser.getParentNodesIDs({
          id,
          array: true
        });
        var resp = [];
        ...
        for (var parent_id of parents) {
          var node = yield _this23.dsl_parser.getNode({
            id: parent_id,
            recurse: false
          });
          var command = yield _this23.findValidCommand({
            node,
            show_debug: false
          });
    ...
    */
    ...
    getParentIDs2Array() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments11.length > 0 && _arguments11[0] !== undefined ? _arguments11[0] : _this24.throwIfMissing('id - ParentIDs2Array');
        return yield _this24.getParentIDs(id, true);
      })();
    ...
    */
    ...
    getParentIDs2ArrayWXID() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments12.length > 0 && _arguments12[0] !== undefined ? _arguments12[0] : _this25.throwIfMissing('id - ParentIDs2ArrayWXID');
        // this is only used in ti.cfc: def_textonly (just for back-compatibility in case needed);
        // @deprecated 4-ago-2020
        var parents = yield _this25.getParentIDs(id, true);
          ...
          ask: function () {
            var _ask = _asyncToGenerator(function* (question) {
              ...
              var resp = (yield prompts({
                type: 'text',
                name: 'value',
                message: _this26.x_console.colorize(question),
                validate: value => {
