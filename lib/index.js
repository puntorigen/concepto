  ...
  class concepto {
    ...
    */
    ...
    generateAutocompleteFiles() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var meta_ = Object.keys(_this2.x_commands.meta)[0]; //x_commands meta data for first lib of x_commands 

        var meta = _this2.x_commands.meta[meta_]; //get x_commands name & language definitions
        ...
        try {
          var concepto_loc = path.dirname(require.resolve('concepto/package.json'));
          iconsPath = path.join(concepto_loc, 'lib', 'export', 'runtime', 'icons');

          _this2.x_console.outT({
            message: 'iconsPath:' + iconsPath
          });

          var copy = require('recursive-copy');

          try {
            yield copy(iconsPath, _this2.autocomplete.path);
          } catch (ercp) {}
        } catch (eeeec) {
          _this2.x_console.outT({
        ...
        */
        ...
        var attributesToHTMLTable = function attributesToHTMLTable() {
          ...
          var html = "<table width='400' border=1 cellspacing=0 cellpadding=2 bordercolor='#AAD3F3'>"; //table header

          html += "<tr bgcolor='#AAD3F3'>\n\t\t\t<td colspan='4' valign='top' align='left'>".concat(poKeys[lang]['Attributes'], ":</td>\n\t\t\t</tr><tr bgcolor='#AAD3F3'>\n\t\t\t<td valign='top' width='10'>R</td>\n\t\t\t<td valign='top'>").concat(poKeys[lang]['Attribute'], "</td>\n\t\t\t<td valign='top'>").concat(poKeys[lang]['Values'], "</td>\n\t\t\t<td valign='top'>").concat(poKeys[lang]['Hint'], "</td>\n\t\t\t</tr><tr>"); //

          for (var key in attributes) {
            var values = attributes[key].values;
            var hint = attributes[key].hint;
            html += "<tr bgcolor='#AAD3F3'>";
            ...
            html += "<td>".concat(key, "</td><td>").concat(values, "</td><td>").concat(hint, "</td>");
            html += "</tr>";
          }
        ...
        var generateKeywordXML = /*#__PURE__*/function () {
          var _ref3 = _asyncToGenerator(function* (record) {
            var keyword = record.text;
            var attributes = record.attributes;
            var html_attr = attributesToHTMLTable(attributes);
            var xml = "\t\t<keyword type=\"other\" name=\"".concat(keyword, "\">\n");
            html = "<BASE href=\"file://".concat(iconsPath, "\">\n<img src=\"idea.png\" align=\"left\" hspace=\"5\" vspace=\"5\" valign=\"middle\" />&nbsp;");
            html += "".concat(keyword.replaceAll('\n', '<br/>'), "<br/>").concat(record.hint.replaceAll('\n', '<br/>'), "<br/>") + html_attr;
            xml += "\t\t\t<desc><![CDATA[\n".concat(html, "\n]]>\n</desc>\n");
            xml += "\t\t</keyword>\n";
            return xml;
          });
        ...
        for (var hash_ in _this2.autocomplete.records) {
          var hash = _this2.autocomplete.records[hash_]; //this.x_console.outT({ message:`hash_ data ({hash_,hash})`, prefix:'autocomplete', data:{hash_,hash} });
          ...
          for (var file_ of files) {
            ...
            if (file_ != undefined) {
              var file = path.join(_this2.autocomplete.path, file_ + '.xml'); //this.x_console.outT({ message:`full -> ${file}`, prefix:'autocomplete' });
              //check if file exists and if it contains keyword[name=hash.text]

              var exists = yield _this2.exists(file); //this.x_console.outT({ message:`exists -> ${exists}`, prefix:'autocomplete' });

              if (exists == true) {
                //if file exists, see if it contains the keyword
                var alreadyExists = yield existsKeyword(hash.text, file); //this.x_console.outT({ message:`existsKeyword -> ${alreadyExists}`, prefix:'autocomplete' });

                if (alreadyExists == false) {
                  //add keyword to existing file (using cheerio)
                  yield addKeywordToXML(yield generateKeywordXML(hash), file);
                }
              } else {
                ...
                yield fs.writeFile(file, newXML);
              }
            }
          }
        }
    ...
    addAutocompleteDefinition() {
      ...
      return _asyncToGenerator(function* () {
        ...
        */
        ...
        var hash = yield _this3.dsl_parser.hash(_arguments);
        _this3.autocomplete.records[hash] = {
          keys,
          bestKey: key,
          text,
          icons,
        ...
        return _this3.autocomplete.records[hash];
      })();
    } // **********************************
    ...
    addCommands(command_func) {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (command_func && typeof command_func === 'function') {
          var t = yield command_func(_this5);

          if (typeof t === 'object') {
            // if there was a meta, add new info as name child
            if (_this5.x_commands && _this5.x_commands.meta && t.meta && t.meta.name) {
              if (!_this5.x_commands.meta.other) _this5.x_commands.meta.other = {};
              _this5.x_commands.meta.other[t.meta.name] = t.meta;
              delete t.meta;
            }

            _this5.x_commands = _objectSpread2(_objectSpread2({}, _this5.x_commands), t);
          } else {
          ...
          // if there was a meta, add new info as name child
          if (_this5.x_commands && _this5.x_commands.meta && command_func.meta && command_func.meta.name) {
            if (!_this5.x_commands.meta.other) _this5.x_commands.meta.other = {};
            _this5.x_commands.meta.other[command_func.meta.name] = command_func.meta;
            delete command_func.meta;
          }

          _this5.x_commands = _objectSpread2(_objectSpread2({}, _this5.x_commands), command_func);
    ...
    */
    ...
    cacheCommands() {
      ...
      return _asyncToGenerator(function* () {
        ...
        for (var x in _this6.x_commands) {
          if (_this6.x_commands[x].x_watch) {
            var watched = _this6.x_commands[x].x_watch.split(',');
            ...
            for (var xi in watched) {
              ...
              if (key.includes('x_state.')) {
                ...
                try {
                  watched_vars[x][key.trim()] = safe('this.' + key.trim(), _this6);
                } catch (doesnt_exist) {
          ...
          if (x == 'meta') {
            x_cmds_hashes[x] = yield _this6.dsl_parser.hash(JSON.stringify(_this6.x_commands[x]));
          } else {
            x_cmds_hashes[x] = yield _this6.dsl_parser.hash(_this6.x_commands[x].func.toString());
          }
        } //console.log('obj_to_hash',x_cmds_hashes);


        var commands_hash = yield _this6.dsl_parser.hash(x_cmds_hashes);
        var commands_cache = yield _this6.cache.getItem('commands_hash');
        var commands_cached_hashes = yield _this6.cache.getItem('commands_hashes');
        var changed_x_cmds = [];
        var watched_vars_hash_now = yield _this6.dsl_parser.hash(watched_vars);
        var watched_vars_hash_cache = yield _this6.cache.getItem('watched_vars_cache');
        var watched_vars_cache = yield _this6.cache.getItem('watched_vars'); //console.log('PABLO debug: watched_vars_now',{current:watched_vars,cache:watched_vars_cache});
        ...
        if (commands_cache != commands_hash || watched_vars_hash_now != watched_vars_hash_cache) {
          if (typeof commands_cached_hashes === 'object') {
            ...
            for (var _x6 in x_cmds_hashes) {
              if (_x6 in commands_cached_hashes && commands_cached_hashes[_x6] != x_cmds_hashes[_x6]) {
                changed_x_cmds.push(_x6); // search x within x_watches, if x is found, also add that cmd (value) to be cleaned from cache

                if (_x6 in x_watches) {
                  if (!changed_x_cmds.includes(x_watches[_x6])) {
                    changed_x_cmds.push(x_watches[_x6]);
                  }
                }
              ...
              if (watched_vars_cache && _x6 in watched_vars_cache && _x6 in watched_vars) {
                for (var varkey in watched_vars_cache[_x6]) {
                  if (watched_vars[_x6][varkey] != watched_vars_cache[_x6][varkey]) {
                    _this6.x_console.outT({
                      prefix: 'cache,yellow',
                      message: "watched var value by ".concat(_x6, " changed! requesting rebuild of its instances"),
                      color: 'brightYellow'
                    });

                    changed_x_cmds.push(_x6);
                  }
                }
              }
            }
            ...
            if (changed_x_cmds.includes('meta')) {
              ...
              yield _this6.cache.clear();
            } else {
              if (changed_x_cmds.length > 0) _this6.x_console.outT({
                prefix: 'cache,yellow',
              ...
              var meta_cache = yield _this6.cache.getItem('meta_cache');
              var amount_cleaned = 0;

              if (meta_cache && typeof meta_cache === 'object' && Object.keys(meta_cache).length > 0) {
                for (var _x7 in meta_cache) {
                  if (_this6.array_intersect(meta_cache[_x7].x_ids.split(','), changed_x_cmds).length > 0) {
                    //remove page 'hashkey' from cache
                    _this6.x_console.outT({
                      prefix: 'cache,yellow',
                      message: "removing ".concat(_x7, " file info from cache .."),
                      color: 'dim'
                    });

                    yield _this6.cache.removeItem(meta_cache[_x7].cachekey);
                    yield _this6.cache.removeItem(meta_cache[_x7].cachekey + '_x_state');
                    amount_cleaned += 1;
                  }
                }
          ...
          yield _this6.cache.setItem('watched_vars', watched_vars);
          yield _this6.cache.setItem('watched_vars_hash_now', watched_vars_hash_now);
          yield _this6.cache.setItem('commands_hash', commands_hash);
          yield _this6.cache.setItem('commands_hashes', x_cmds_hashes);
        }
      })();
    }
    /**
    ...
    */
    ...
    findCommand() {
      ...
      return _asyncToGenerator(function* () {
        var {
          node = _this7.throwIfMissing('node'),
          justone = true,
          show_debug = true
        ...
        var resp = _objectSpread2(_objectSpread2({}, _this7.reply_template()), {
          id: 'not_found',
        ...
        if (typeof node.icons === 'undefined') {
          if (show_debug) _this7.debug('error: findCommand was given a blank node!');
          return resp;
        }

        var cache_key = node.id + node.date_modified.toString(); //+node.nodes_raw.toString();

        var t_cache = yield _this7.cache.getItem(cache_key);

        if (t_cache && (Array.isArray(t_cache) && t_cache.length > 0 || t_cache.data)) {
          // node.id in this.x_memory_cache.findCommand
          if (show_debug) _this7.debug("using memory_cache for findCommand for node ID ".concat(node.id));
          return t_cache;
        } else {
          if (show_debug) _this7.debug("findCommand for node ID ".concat(node.id));
          ...
          delete _this7.x_commands['constructor']; //4-jun-21 @todo double check this; maybe we should use a cloned var
          // iterate through commands

          for (var key in _this7.x_commands) {
            ...
            var command_requires = _objectSpread2(_objectSpread2({}, command_defaults), _this7.x_commands[key]);
            ...
            if (command_requires['x_not_icons'] != '' && allTrue(matched, keys)) {
              ...
              if (node.icons.length > 0 && command_requires['x_not_icons'] != '' && ['*'].includes(command_requires['x_not_icons'])) {
                ...
                matched.x_not_icons = _this7.array_intersect(command_requires['x_not_icons'].split(','), node.icons).length > 0 ? false : true;
              }
              ...
              for (var test of com_reqs) {
                ...
                if (test.indexOf('.') != -1) {
                  ...
                  (function () {
                    ...
                    var keys = _this7.dsl_parser.findVariables({
                      text: test,
                      symbol: '[',
                      symbol_closing: ']'
                    }).split('+');
                    ...
                    if (_this7.array_intersect(has_keys, keys).length != keys.length) {
                      matched.x_not_empty = false;
                    }
                  })();
                } else {
            ...
            if (command_requires['x_or_hasparent'] != '' && allTrue(matched, keys)) {
              ...
              matched.x_or_hasparent = yield _this7.hasParentID(node.id, command_requires['x_or_hasparent']);
            ...
            if (command_requires['x_all_hasparent'] != '' && allTrue(matched, keys)) {
              ...
              matched.x_all_hasparent = yield _this7.hasParentID(node.id, command_requires['x_all_hasparent'], true);
            ...
            if (command_requires['x_or_isparent'] != '' && allTrue(matched, keys)) {
              ...
              for (var _key7 of command_requires['x_or_isparent'].split(',')) {
                is_direct = yield _this7.isExactParentID(node.id, _key7);
            ...
            if (typeof command_requires['x_text_pattern'] === 'string' && command_requires['x_text_pattern'] != '' && allTrue(matched, keys)) {
              ...
              matched.x_text_pattern = _this7.x_match(node.text.trim(), command_requires['x_text_pattern']);
            ...
            if (Array.isArray(command_requires['x_text_pattern']) == true && command_requires['x_text_pattern'].length > 0 && allTrue(matched, keys)) {
              ...
              for (var xtp of command_requires['x_text_pattern']) {
                _test = _this7.x_match(text_trim, xtp);
                if (_test == true) break;
              }
          ...
          if (show_debug) _this7.debug_time({
          ...
          if (show_debug) _this7.debug_timeEnd({
            id: "sorting by priority"
          }); // reply
          ...
          if (Array.isArray(resp) && resp.length > 0 || resp.data) {
            ...
            yield _this7.cache.setItem(cache_key, resp);
    ...
    */
    findValidCommand() {
      ...
      return _asyncToGenerator(function* () {
        var {
          node = _this9.throwIfMissing('node'),
          object = false,
          x_command_shared_state = {},
          show_debug = true
        } = _arguments3.length > 0 && _arguments3[0] !== undefined ? _arguments3[0] : {};
        if (!_this9.x_flags.init_ok) throw new Error('error! the first called method must be init()!');

        if (node.id in _this9.x_memory_cache.findValidCommand) {
          ...
          return _this9.x_memory_cache.findValidCommand[node.id];
        } else {
          if (show_debug) _this9.debug({
          ...
          var commands_ = yield _this9.findCommand({
            node,
            justone: false,
          ...
          if (commands_.length == 0) {
            ...
            try {
              //if (object==true) {
              var test = yield _this9.x_commands[reply.x_id].func(node, x_command_shared_state);
              reply.exec = test; //}
              ...
              yield _this9.showLineError(test_err); // @TODO emit('internal_error','findValidCommand')
            ...
            for (var qm_index in commands_) {
              ...
              if (_this9.x_commands[qm.x_id]) {
                try {
                  var _test2 = yield _this9.x_commands[qm.x_id].func(node, x_command_shared_state);
                  ...
                  yield _this9.showLineError(test_err1);
                  reply.error = true;
                  reply.valid = false;
          ...
          _this9.x_memory_cache.findValidCommand[node.id] = reply;
          return reply;
        }
      })();
    } // ****************************
    ...
    */
    ...
    process() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var x_dsl_nodes = yield _this10.dsl_parser.getNodes({
          level: 2,
          nodes_raw: true,
          hash_content: true
        }); // add abort support
        ...
        _this10.debug('calling onPrepare');

        _this10.debug_time({
          id: 'onPrepare'
        });

        yield _this10.onPrepare();
        ...
        if (!_this10.x_config.debug && !_this10.x_config.silent) {
          _this10.progress_multi = {};
          _this10.multibar = _this10.x_console.progress({
          ...
          _this10.progress_multi['_total_'] = _this10.multibar.create(x_dsl_nodes.length - 1, {
            total_: 'x',
            screen: 'initializing..'
          });
        }
        ...
        var counter_ = 0;
        var meta_cache = {};
        var meta_cache_ = yield _this10.cache.getItem('meta_cache');
        ...
        for (var level2 of x_dsl_nodes) {
          ...
          //break;
          if (!_this10.x_config.debug && !_this10.x_config.silent) {
            if (_this10.progress_last) _this10.progress_last.raw().stop();
            _this10.progress_multi[level2.text] = _this10.multibar.create(level2.nodes_raw.length - 1, {
              total_: '',
              screen: 'initializing..',
              error: false
            });

            _this10.progress_multi['_total_'].update(counter_, {
              total_: 'x',
              screen: level2.text,
              error: false
            });

            _this10.progress_last = _this10.progress_multi[level2.text];
            _this10.progress_last_screen = level2.text;
          }

          if (_this10.abort && !_this10.x_config.debug && !_this10.x_config.silent) {
            _this10.progress_multi[level2.text].raw().stop();

            break; //abort processing if request
          } else if (_this10.abort) {
            break; //abort processing if request
          } //cache: check if current node has any children that were modified since last time


          var main = yield _this10.cache.getItem(level2.hash_content); // remove await when in production (use Promise.all after loop then)

          if (!main) {
            var before_state = JSON.parse(JSON.stringify(_this10.x_state));
            main = yield _this10.process_main(level2, {});
            var state_to_save = obj_diff(before_state, _this10.x_state); //console.log('state_to_save',{ state_to_save });
            ...
            if (main.error && main.error == true) {
              ...
              yield _this10.cache.setItem(level2.hash_content, main);
              yield _this10.cache.setItem(level2.hash_content + '_x_state', state_to_save);
            } //console.log('metido al cache:'+level2.id,main);

          } else {
            var cached_state = yield _this10.cache.getItem(level2.hash_content + '_x_state');
            _this10.x_state = deep.all([_this10.x_state, cached_state]);

            if (main.error && main.error == true) {
              yield _this10.cache.removeItem(level2.hash_content); //reprocess the removed failed cached node.

              var _before_state = JSON.parse(JSON.stringify(_this10.x_state));

              main = yield _this10.process_main(level2, {});

              var _state_to_save = obj_diff(_before_state, _this10.x_state); //console.log('state_to_save2',{ state_to_save });
              ...
              if (main.error && main.error == true) ; else {
                ...
                yield _this10.cache.setItem(level2.hash_content, main);
                yield _this10.cache.setItem(level2.hash_content + '_x_state', _state_to_save);
              }
          ...
          if (!_this10.x_config.debug && !_this10.x_config.silent) {
            if (main.error && main.error == true) {
              ...
              _this10.progress_multi[level2.text].total(level2.nodes_raw.length - 1);

              _this10.progress_multi[level2.text].update(level2.nodes_raw.length - 1, {
                screen: level2.text,
                sub: '',
                total_: ''
              });
        ...
        if (!_this10.x_config.debug && !_this10.x_config.silent) {
          _this10.progress_multi['_total_'].raw().stop(); //.remove();


          _this10.multibar.stop();
        ...
        yield _this10.generateAutocompleteFiles(); // if there was no error

        if (!were_errors && !_this10.abort) {
          // request creation of files
          yield _this10.onCreateFiles(resp.nodes);

          _this10.x_console.title({
            title: "Interpreter ".concat(_this10.x_config.class.toUpperCase(), " ENDED. Full Compilation took: ").concat(_this10.secsPassed_()),
            color: 'green'
          });

          _this10.debug_table('Amount of Time Per Command');
        } else if (_this10.abort) {
          _this10.x_console.title({
            title: "Interpreter ".concat(_this10.x_config.class.toUpperCase(), " ABORTED. Partial Compilation took: ").concat(_this10.secsPassed_()),
            color: 'green'
          });
        } else {
          // errors occurred
          _this10.x_console.title({
            title: "Interpreter ".concat(_this10.x_config.class.toUpperCase(), " ENDED with ERRORS.\nPlease check your console history.\nCompilation took: ").concat(_this10.secsPassed_()),
        ...
        yield _this10.cache.setItem('last_compile_date', new Date()); //add meta cache to cache

        yield _this10.cache.setItem('meta_cache', meta_cache);
        yield _this10.onEnd();
        return resp;
      })();
      ...
      return _asyncToGenerator(function* () {
        ...
        if (resp.hasChildren == true && resp.valid == true && nodei.getNodes) {
          ...
          if (!_this11.x_config.debug && !_this11.x_config.silent) {
            _this11.progress_last.total(sub_nodes.length - 1); //this.progress_multi['_total_'].raw().updateETA();
          ...
          for (var sublevel of sub_nodes) {
            ...
            var real = yield _this11.dsl_parser.getNode({
              id: sublevel.id,
              nodes_raw: true,
              recurse: false
            }); //this.debug('sub_process before findValidCommand node id:'+real.id);

            var real2 = yield _this11.findValidCommand({
              node: real,
              object: false,
            ...
            if (!_this11.x_config.debug && !_this11.x_config.silent) {
              _this11.progress_last.update(xx, {
                screen: _this11.progress_last_screen,
                sub: real2.x_id,
                total_: ''
              });
            ...
            if (real2 && real2.exec && real2.exec.valid == true) {
              ...
              resp = yield _this11.sub_process(resp, sublevel, new_state);
              resp.code += real2.exec.close;
            } else if (real2.error == true) {
              if (!_this11.x_config.debug && !_this11.x_config.silent) {
                _this11.progress_last.total(xx);
              ...
              yield _this11.onErrors(["Error executing func for x_command:".concat(real2.x_id, " for node id ").concat(real.id, ", text: ").concat(real.text, " ")]);
    ...
    showLineError(error) {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (error_info.file && (yield _this12.exists(error_info.file)) == true) {
          var fs = require('fs').promises;
    ...
    process_main(node, custom_state) {
      ...
      return _asyncToGenerator(function* () {
        var resp = {
          state: custom_state,
          id: node.id,
          name: yield _this13.onDefineNodeName(node),
          file: yield _this13.onDefineFilename(node),
          init: '',
          title: yield _this13.onDefineTitle(node),
        ...
        var test = yield _this13.findValidCommand({
          node: node,
          object: false,
        ...
        if (test && test.exec && test.exec.valid == true) {
          ...
          if (typeof node.getNodes === 'function') {
            resp = yield _this13.sub_process(resp, node, copy_state);
          }
          ...
          yield _this13.onErrors(["Error executing func for x_command:".concat(test.x_id, " for node id ").concat(node.id, ", text: ").concat(node.text, " ")]);

          if (!_this13.x_config.debug && !_this13.x_config.silent) {
            _this13.progress_last.total(2);

            _this13.progress_last.update(1, {
              screen: _this13.progress_last_screen + '\n',
              error: true
            });
          } // improved error logging


          yield _this13.showLineError(test.catch);
          resp.valid = false, resp.hasChildren = false, resp.error = true;
        } else {
          _this13.x_console.outT({
          ...
          yield _this13.onErrors(["No method found for given node id ".concat(node.id, ", text: ").concat(node.text, " ")]);
          resp.valid = false, resp.hasChildren = false, resp.error = true;
        } // closing level2 'on' calls


        resp = yield _this13.onAfterProcess(resp);
        resp = yield _this13.onCompleteCodeTemplate(resp); //
    ...
    */
    ...
    _appFolders(keys) {
      ...
      return _asyncToGenerator(function* () {
        var compile_folder = _arguments4.length > 1 && _arguments4[1] !== undefined ? _arguments4[1] : _this14.x_state.central_config.apptitle;
        var output_dir = _arguments4.length > 2 ? _arguments4[2] : undefined;

        var fs = require('fs').promises;

        _this14.debug('_appFolders');

        var path = require('path');

        var dsl_folder = path.dirname(path.resolve(_this14.x_flags.dsl)) + path.sep;
        if (output_dir) dsl_folder = output_dir;
        var resp = {
          base: dsl_folder,
          src: dsl_folder + (compile_folder ? compile_folder : _this14.x_state.central_config.apptitle) + path.sep
        };
    ...
    */
    ...
    hasBrotherID() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments5.length > 0 && _arguments5[0] !== undefined ? _arguments5[0] : _this15.throwIfMissing('id-brotherID');
        var x_id = _arguments5.length > 1 && _arguments5[1] !== undefined ? _arguments5[1] : _this15.throwIfMissing('x_id');

        // @TODO test it after having 'real' commands on some parser 3-ago-20
        if (id + x_id in _this15.x_memory_cache.hasBrotherID) {
          return _this15.x_memory_cache.hasBrotherID[id + x_id];
        } else {
          var brother_ids = yield _this15.dsl_parser.getBrotherNodesIDs({
            id,
            before: true,
            after: true
          }).split(',');
          ...
          _this15.x_memory_cache.hasBrotherID[id + x_id] = resp;
          return resp;
        }
      })();
    }
    ...
    */
    ...
    hasBrotherBefore() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments6.length > 0 && _arguments6[0] !== undefined ? _arguments6[0] : _this16.throwIfMissing('id-brotherBefore');

        if (id in _this16.x_memory_cache.hasBrotherBefore) {
          return _this16.x_memory_cache.hasBrotherBefore[id];
        } else {
          ...
          _this16.x_memory_cache.hasBrotherBefore[id] = brother_ids.includes(id);
          return _this16.x_memory_cache.hasBrotherBefore[id];
        }
      })();
    }
    ...
    */
    ...
    hasBrotherNext() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments7.length > 0 && _arguments7[0] !== undefined ? _arguments7[0] : _this17.throwIfMissing('id - BrotherNext');

        if (id in _this17.x_memory_cache.hasBrotherNext) {
          return _this17.x_memory_cache.hasBrotherNext[id];
        } else {
          ...
          _this17.x_memory_cache.hasBrotherNext[id] = brother_ids.includes(id);
          return _this17.x_memory_cache.hasBrotherNext[id];
        }
      })();
    }
    ...
    */
    ...
    isExactParentID() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments8.length > 0 && _arguments8[0] !== undefined ? _arguments8[0] : _this18.throwIfMissing('id-ExactParent');
        var x_id = _arguments8.length > 1 && _arguments8[1] !== undefined ? _arguments8[1] : _this18.throwIfMissing('x_id');

        // @TODO test it after having 'real' commands on some parser 4-ago-20
        if (id + x_id in _this18.x_memory_cache.isExactParentID) {
          return _this18.x_memory_cache.isExactParentID[id + x_id];
        } else {
          var parent_node = yield _this18.dsl_parser.getParentNode({
            id
          });
          var parent_command = yield _this18.findValidCommand({
            node: parent_node,
            show_debug: false,
            object: true
          });
          ...
          if (parent_command && parent_command.x_id == x_id) {
            _this18.x_memory_cache.isExactParentID[id + x_id] = true;
            return true;
          }

          _this18.x_memory_cache.isExactParentID[id + x_id] = false;
          return false;
        }
      })();
    }
    ...
    */
    ...
    hasParentID() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments9.length > 0 && _arguments9[0] !== undefined ? _arguments9[0] : _this19.throwIfMissing('id-ParentID');
        var x_id = _arguments9.length > 1 && _arguments9[1] !== undefined ? _arguments9[1] : _this19.throwIfMissing('x_id');
        ...
        var parents = yield _this19.dsl_parser.getParentNodesIDs({
          id,
          array: true
        });
        ...
        for (var parent_id of parents) {
          var node = yield _this19.dsl_parser.getNode({
            id: parent_id,
            recurse: false
          });
          var parentCommand = yield _this19.findValidCommand({
            node,
            show_debug: false,
            object: true
          });
          ...
          if (onlyTrueIfAll == false && x_ids.includes(parentCommand.x_id)) {
            ...
            if (_this19.array_intersect(tested_parents_x_ids, x_ids).length == x_ids.length) {
              return true;
            }
          }
    ...
    */
    ...
    getParentIDs() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments10.length > 0 && _arguments10[0] !== undefined ? _arguments10[0] : _this20.throwIfMissing('id-ParentIDs');
        ...
        for (var parent_id of parents) {
          ...
          var command = yield _this20.findValidCommand({
            node,
            show_debug: false
          });
    ...
    */
    ...
    getParentIDs2Array() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments11.length > 0 && _arguments11[0] !== undefined ? _arguments11[0] : _this21.throwIfMissing('id - ParentIDs2Array');
        return yield _this21.getParentIDs(id, true);
      })();
    ...
    */
    ...
    getParentIDs2ArrayWXID() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments12.length > 0 && _arguments12[0] !== undefined ? _arguments12[0] : _this22.throwIfMissing('id - ParentIDs2ArrayWXID');
        // this is only used in ti.cfc: def_textonly (just for back-compatibility in case needed);
        // @deprecated 4-ago-2020
        var parents = yield _this22.getParentIDs(id, true);
