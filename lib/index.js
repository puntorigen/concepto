        ...
        */
        ...
        if (jsonKeys.length > 0) {
          ...
          for (var key of jsonKeys) {
            var record = _this3.autocomplete.json[key];

            if (record.hint) {
              record.hint = yield _this3.autocompleteContentTemplate(record, {
                icon: _icon2 => {
                  return "<img src=\"".concat(_icon2, ".png\" align=\"left\" hspace=\"5\" vspace=\"5\" valign=\"middle\" />&nbsp;");
                },
                placeholders: (text, renderIcon) => {
                  if (renderIcon) {
                    //return attributesToHTMLTable(attrs,renderIcon);
                ...
                attrs: (_attrs2, renderIcon) => {
                  if (renderIcon) {
                    return attributesToHTMLTable(_attrs2, renderIcon);
                  } else {
                    return attributesToHTMLTable(_attrs2, default_render_icon);
                  }
                }
              });
            }

            _this3.autocomplete.json[key] = record; //this may not be needed anymore
