  ...
  class concepto {
    ...
    */
    findValidCommand() {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (node.id in _this7.x_memory_cache.findValidCommand) {
          ...
          return _this7.x_memory_cache.findValidCommand[node.id];
        } else {
          if (show_debug) _this7.debug({
          ...
          var commands_ = yield _this7.findCommand({
            node,
            justone: false,
          ...
          if (commands_.length == 0) {
            ...
            try {
              //if (object==true) {
              var test = yield _this7.x_commands[reply.x_id].func(node, x_command_shared_state);
              reply.exec = test; //}
              ...
              yield _this7.showLineError(test_err); // @TODO emit('internal_error','findValidCommand')
            ...
            for (var qm_index in commands_) {
              ...
              if (_this7.x_commands[qm.x_id]) {
                try {
                  var _test2 = yield _this7.x_commands[qm.x_id].func(node, x_command_shared_state);
                  ...
                  yield _this7.showLineError(test_err1);
                  reply.error = true;
                  reply.valid = false;
          ...
          _this7.x_memory_cache.findValidCommand[node.id] = reply;
          return reply;
        }
      })();
    } // ****************************
    ...
    */
    ...
    process() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var x_dsl_nodes = yield _this8.dsl_parser.getNodes({
          level: 2,
          nodes_raw: true,
          hash_content: true
        }); // add abort support
        ...
        _this8.debug('calling onPrepare');

        _this8.debug_time({
          id: 'onPrepare'
        });

        yield _this8.onPrepare();
        ...
        if (!_this8.x_config.debug && !_this8.x_config.silent) {
          _this8.progress_multi = {};
          _this8.multibar = _this8.x_console.progress({
          ...
          _this8.progress_multi['_total_'] = _this8.multibar.create(x_dsl_nodes.length - 1, {
            total_: 'x',
            screen: 'initializing..'
          });
        }
        ...
        var counter_ = 0;
        var meta_cache = {};
        var meta_cache_ = yield _this8.cache.getItem('meta_cache');
        ...
        for (var level2 of x_dsl_nodes) {
          ...
          //break;
          if (!_this8.x_config.debug && !_this8.x_config.silent) {
            if (_this8.progress_last) _this8.progress_last.raw().stop();
            _this8.progress_multi[level2.text] = _this8.multibar.create(level2.nodes_raw.length - 1, {
              total_: '',
              screen: 'initializing..',
              error: false
            });

            _this8.progress_multi['_total_'].update(counter_, {
              total_: 'x',
              screen: level2.text,
              error: false
            });

            _this8.progress_last = _this8.progress_multi[level2.text];
            _this8.progress_last_screen = level2.text;
          }

          if (_this8.abort && !_this8.x_config.debug && !_this8.x_config.silent) {
            _this8.progress_multi[level2.text].raw().stop();

            break; //abort processing if request
          } else if (_this8.abort) {
            break; //abort processing if request
          } //cache: check if current node has any children that were modified since last time


          var main = yield _this8.cache.getItem(level2.hash_content); // remove await when in production (use Promise.all after loop then)

          if (!main) {
            var before_state = JSON.parse(JSON.stringify(_this8.x_state));
            main = yield _this8.process_main(level2, {});
            var state_to_save = obj_diff(before_state, _this8.x_state); //console.log('state_to_save',{ state_to_save });
            ...
            if (main.error && main.error == true) {
              ...
              yield _this8.cache.setItem(level2.hash_content, main);
              yield _this8.cache.setItem(level2.hash_content + '_x_state', state_to_save);
            } //console.log('metido al cache:'+level2.id,main);

          } else {
            var cached_state = yield _this8.cache.getItem(level2.hash_content + '_x_state');
            _this8.x_state = deep.all([_this8.x_state, cached_state]);

            if (main.error && main.error == true) {
              yield _this8.cache.removeItem(level2.hash_content); //reprocess the removed failed cached node.

              var _before_state = JSON.parse(JSON.stringify(_this8.x_state));

              main = yield _this8.process_main(level2, {});

              var _state_to_save = obj_diff(_before_state, _this8.x_state); //console.log('state_to_save2',{ state_to_save });
              ...
              if (main.error && main.error == true) ; else {
                ...
                yield _this8.cache.setItem(level2.hash_content, main);
                yield _this8.cache.setItem(level2.hash_content + '_x_state', _state_to_save);
              }
          ...
          if (!_this8.x_config.debug && !_this8.x_config.silent) {
            if (main.error && main.error == true) {
              ...
              _this8.progress_multi[level2.text].total(level2.nodes_raw.length - 1);

              _this8.progress_multi[level2.text].update(level2.nodes_raw.length - 1, {
                screen: level2.text,
                sub: '',
                total_: ''
              });
        ...
        if (!_this8.x_config.debug && !_this8.x_config.silent) {
          _this8.progress_multi['_total_'].raw().stop(); //.remove();


          _this8.multibar.stop();
        ...
        if (!were_errors && !_this8.abort) {
          // request creation of files
          yield _this8.onCreateFiles(resp.nodes);

          _this8.x_console.title({
            title: "Interpreter ".concat(_this8.x_config.class.toUpperCase(), " ENDED. Full Compilation took: ").concat(_this8.secsPassed_()),
            color: 'green'
          });

          _this8.debug_table('Amount of Time Per Command');
        } else if (_this8.abort) {
          _this8.x_console.title({
            title: "Interpreter ".concat(_this8.x_config.class.toUpperCase(), " ABORTED. Partial Compilation took: ").concat(_this8.secsPassed_()),
            color: 'green'
          });
        } else {
          // errors occurred
          _this8.x_console.title({
            title: "Interpreter ".concat(_this8.x_config.class.toUpperCase(), " ENDED with ERRORS.\nPlease check your console history.\nCompilation took: ").concat(_this8.secsPassed_()),
        ...
        yield _this8.cache.setItem('last_compile_date', new Date()); //add meta cache to cache

        yield _this8.cache.setItem('meta_cache', meta_cache);
        yield _this8.onEnd();
        return resp;
      })();
    ...
    sub_process(source_resp, nodei, custom_state) {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (resp.hasChildren == true && resp.valid == true && nodei.getNodes) {
          ...
          if (!_this9.x_config.debug && !_this9.x_config.silent) {
            _this9.progress_last.total(sub_nodes.length - 1); //this.progress_multi['_total_'].raw().updateETA();
          ...
          for (var sublevel of sub_nodes) {
            ...
            var real = yield _this9.dsl_parser.getNode({
              id: sublevel.id,
              nodes_raw: true,
              recurse: false
            }); //this.debug('sub_process before findValidCommand node id:'+real.id);

            var real2 = yield _this9.findValidCommand({
              node: real,
              object: false,
            ...
            if (!_this9.x_config.debug && !_this9.x_config.silent) {
              _this9.progress_last.update(xx, {
                screen: _this9.progress_last_screen,
                sub: real2.x_id,
                total_: ''
              });
            ...
            if (real2 && real2.exec && real2.exec.valid == true) {
              ...
              resp = yield _this9.sub_process(resp, sublevel, new_state);
              resp.code += real2.exec.close;
            } else if (real2.error == true) {
              if (!_this9.x_config.debug && !_this9.x_config.silent) {
                _this9.progress_last.total(xx);
              ...
              yield _this9.onErrors(["Error executing func for x_command:".concat(real2.x_id, " for node id ").concat(real.id, ", text: ").concat(real.text, " ")]);
    ...
    showLineError(error) {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (error_info.file && (yield _this10.exists(error_info.file)) == true) {
          var fs = require('fs').promises;
    ...
    process_main(node, custom_state) {
      ...
      return _asyncToGenerator(function* () {
        var resp = {
          state: custom_state,
          id: node.id,
          name: yield _this11.onDefineNodeName(node),
          file: yield _this11.onDefineFilename(node),
          init: '',
          title: yield _this11.onDefineTitle(node),
        ...
        var test = yield _this11.findValidCommand({
          node: node,
          object: false,
        ...
        if (test && test.exec && test.exec.valid == true) {
          ...
          if (typeof node.getNodes === 'function') {
            resp = yield _this11.sub_process(resp, node, copy_state);
          }
          ...
          yield _this11.onErrors(["Error executing func for x_command:".concat(test.x_id, " for node id ").concat(node.id, ", text: ").concat(node.text, " ")]);

          if (!_this11.x_config.debug && !_this11.x_config.silent) {
            _this11.progress_last.total(2);

            _this11.progress_last.update(1, {
          ...
          yield _this11.showLineError(test.catch);
          resp.valid = false, resp.hasChildren = false, resp.error = true;
        } else {
          _this11.x_console.outT({
          ...
          yield _this11.onErrors(["No method found for given node id ".concat(node.id, ", text: ").concat(node.text, " ")]);
          resp.valid = false, resp.hasChildren = false, resp.error = true;
        } // closing level2 'on' calls


        resp = yield _this11.onAfterProcess(resp);
        resp = yield _this11.onCompleteCodeTemplate(resp); //
    ...
    */
    ...
    _appFolders(keys) {
      ...
      return _asyncToGenerator(function* () {
        var compile_folder = _arguments3.length > 1 && _arguments3[1] !== undefined ? _arguments3[1] : _this12.x_state.central_config.apptitle;
        var output_dir = _arguments3.length > 2 ? _arguments3[2] : undefined;

        var fs = require('fs').promises;

        _this12.debug('_appFolders');

        var path = require('path');

        var dsl_folder = path.dirname(path.resolve(_this12.x_flags.dsl)) + path.sep;
        if (output_dir) dsl_folder = output_dir;
        var resp = {
          base: dsl_folder,
          src: dsl_folder + (compile_folder ? compile_folder : _this12.x_state.central_config.apptitle) + path.sep
        };
    ...
    */
    ...
    hasBrotherID() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments4.length > 0 && _arguments4[0] !== undefined ? _arguments4[0] : _this13.throwIfMissing('id-brotherID');
        var x_id = _arguments4.length > 1 && _arguments4[1] !== undefined ? _arguments4[1] : _this13.throwIfMissing('x_id');

        // @TODO test it after having 'real' commands on some parser 3-ago-20
        if (id + x_id in _this13.x_memory_cache.hasBrotherID) {
          return _this13.x_memory_cache.hasBrotherID[id + x_id];
        } else {
          ...
          _this13.x_memory_cache.hasBrotherID[id + x_id] = resp;
          return resp;
        }
      })();
    }
    ...
    */
    ...
    hasBrotherBefore() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments5.length > 0 && _arguments5[0] !== undefined ? _arguments5[0] : _this14.throwIfMissing('id-brotherBefore');

        if (id in _this14.x_memory_cache.hasBrotherBefore) {
          return _this14.x_memory_cache.hasBrotherBefore[id];
        } else {
          ...
          _this14.x_memory_cache.hasBrotherBefore[id] = brother_ids.includes(id);
          return _this14.x_memory_cache.hasBrotherBefore[id];
        }
      })();
    }
    ...
    */
    ...
    hasBrotherNext() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments6.length > 0 && _arguments6[0] !== undefined ? _arguments6[0] : _this15.throwIfMissing('id - BrotherNext');

        if (id in _this15.x_memory_cache.hasBrotherNext) {
          return _this15.x_memory_cache.hasBrotherNext[id];
        } else {
          ...
          _this15.x_memory_cache.hasBrotherNext[id] = brother_ids.includes(id);
          return _this15.x_memory_cache.hasBrotherNext[id];
        }
      })();
    }
    ...
    */
    ...
    isExactParentID() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments7.length > 0 && _arguments7[0] !== undefined ? _arguments7[0] : _this16.throwIfMissing('id-ExactParent');
        var x_id = _arguments7.length > 1 && _arguments7[1] !== undefined ? _arguments7[1] : _this16.throwIfMissing('x_id');

        // @TODO test it after having 'real' commands on some parser 4-ago-20
        if (id + x_id in _this16.x_memory_cache.isExactParentID) {
          return _this16.x_memory_cache.isExactParentID[id + x_id];
        } else {
          var parent_node = yield _this16.dsl_parser.getParentNode({
            id
          });
          var parent_command = yield _this16.findValidCommand({
            node: parent_node,
            show_debug: false,
            object: true
          });
          ...
          if (parent_command && parent_command.x_id == x_id) {
            _this16.x_memory_cache.isExactParentID[id + x_id] = true;
            return true;
          }

          _this16.x_memory_cache.isExactParentID[id + x_id] = false;
          return false;
        }
      })();
    }
    ...
    */
    ...
    hasParentID() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments8.length > 0 && _arguments8[0] !== undefined ? _arguments8[0] : _this17.throwIfMissing('id-ParentID');
        var x_id = _arguments8.length > 1 && _arguments8[1] !== undefined ? _arguments8[1] : _this17.throwIfMissing('x_id');
        ...
        for (var parent_id of parents) {
          ...
          var parentCommand = yield _this17.findValidCommand({
            node,
            show_debug: false,
            object: true
          });
          ...
          if (onlyTrueIfAll == false && x_ids.includes(parentCommand.x_id)) {
            ...
            if (_this17.array_intersect(tested_parents_x_ids, x_ids).length == x_ids.length) {
              return true;
            }
          }
    ...
    */
    ...
    getParentIDs() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments9.length > 0 && _arguments9[0] !== undefined ? _arguments9[0] : _this18.throwIfMissing('id-ParentIDs');
        var array = _arguments9.length > 1 && _arguments9[1] !== undefined ? _arguments9[1] : false;
        // @TODO test it after having 'real' commands on some parser 4-ago-20
        var parents = yield _this18.dsl_parser.getParentNodesIDs({
          id,
          array: true
        });
        var resp = [];
        ...
        for (var parent_id of parents) {
          ...
          var command = yield _this18.findValidCommand({
            node,
            show_debug: false
          });
    ...
    */
    ...
    getParentIDs2Array() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments10.length > 0 && _arguments10[0] !== undefined ? _arguments10[0] : _this19.throwIfMissing('id - ParentIDs2Array');
        return yield _this19.getParentIDs(id, true);
      })();
    ...
    */
    ...
    getParentIDs2ArrayWXID() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments11.length > 0 && _arguments11[0] !== undefined ? _arguments11[0] : _this20.throwIfMissing('id - ParentIDs2ArrayWXID');
        // this is only used in ti.cfc: def_textonly (just for back-compatibility in case needed);
        // @deprecated 4-ago-2020
        var parents = yield _this20.getParentIDs(id, true);
