        ...
        if (!_this.x_flags.init_ok) {
          ...
          if (_this.x_config.diff_from) {
            ...
            var _fs = require('fs').promises,
                _path = require('path'); //read non _git .dsl file source


            var non_git = _path.join(tmp.directory, _path.basename(_this.x_flags.dsl).replace('_git.dsl', '.dsl'));

            var non_git_content = yield _fs.readFile(non_git, 'utf-8'); //read given diff_from file

            var from_dsl = _path.resolve(_this.x_config.diff_from);

            var from_content = yield _fs.readFile(from_dsl, 'utf-8'); //create _diff.dsl file

            var diff_dsl = _path.join(tmp.directory, _path.basename(_this.x_flags.dsl).replace('_git.dsl', '.dsl').replace('.dsl', '_diff.dsl')); //get differences


            var compare = yield _this.dsl_parser.getDifferences(from_content, non_git_content); //

            var diff = {
              content: ''
            }; //parse diff content

            diff.parser = new dsl_parser({
              file: non_git,
              config: {
                cancelled: false,
                debug: false
            ...
            try {
              yield diff.parser.process();
            } catch (d_err) {}

            diff.content = diff.parser.$.html(); //remove all previous clouds from diff.content
            ...
            for (var key in compare.added) {
              diff.content = yield diff.parser.editNode({
                ...
                data: function data(x) {
                  var new_note = "!! ADDED NODE !!";

                  if (x.text_note.trim().trim() != '') {
                    new_note = "".concat(new_note, "\n").concat(x.text_note);
                  }
            ...
            yield _fs.writeFile(diff_dsl, diff.content, 'utf-8');
        ...
        if (t_cache && (Array.isArray(t_cache) && t_cache.length > 0 || t_cache.data)) {
          ...
          for (var key in _this5.x_commands) {
            ...
            if (command_requires['x_empty'] != '' && allTrue(matched, keys)) {
              ...
              for (var _key3 of command_requires['x_empty'].split(',')) {
                var _testpath = getVal(node, _key3);
            ...
            if (allTrue(matched, keys) && command_requires['x_text_contains'] != '') {
              ...
              if (command_requires['x_text_contains'].indexOf('|') != -1) {
                ...
                for (var _key4 of command_requires['x_text_contains'].split('|')) {
                  if (node.text.indexOf(_key4) != -1) {
                    n_match = true;
                    break;
                  }
                }
                ...
                for (var _key5 of command_requires['x_text_contains'].split(',')) {
                  if (node.text.indexOf(_key5) == -1 || _key5 == '' && node.text.indexOf(',') == -1) {
            ...
            if (command_requires['x_or_isparent'] != '' && allTrue(matched, keys)) {
              ...
              for (var _key6 of command_requires['x_or_isparent'].split(',')) {
                is_direct = yield _this5.isExactParentID(node.id, _key6);
