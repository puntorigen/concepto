        ...
        if (!_this.x_flags.init_ok) {
          ...
          if (_this.x_config.diff_from) {
            yield* function* () {
              ...
              var diff_dsl = path.join(tmp.directory, path.basename(_this.x_flags.dsl).replace('_git.dsl', '.dsl').replace('.dsl', '_diff.dsl')); //@todo clean CREATED and MODIFIED dates and FOLDED attributos from from_content and non_git_content, before getting differences
              ...
              var compare = yield _this.dsl_parser.getDifferences(files.from_compare, files.ng_compare); // reparse.... @todo improve!!!! ... this is just a hack to maintain original format and dates without taking those into account for the diff
              ...
              var from = {};
              from.parser = files.from_parser; //21jun21: get parents of each deleted id, if any parent is within the deleted keys, remove the tested id
          ...
          if (_this.x_config.export_html) {
            ...
            var images = [];
            $('img[src]').toArray().map( /*#__PURE__*/function () {
              var _ref = _asyncToGenerator(function* (elem) {
                ...
                if (src.charAt(0) != '.' && images.includes(src) == false) {
                  ...
                  var exists_ = yield this.exists(full_);

                  if (exists_ == true) {
                    images.push(full_);

                    try {
                      yield copy(full, target_path);
                    } catch (ercp) {}
                  }
                }
              });
      ...
      return _asyncToGenerator(function* () {
        ...
        if (commands_cache != commands_hash || watched_vars_hash_now != watched_vars_hash_cache) {
          if (typeof commands_cached_hashes === 'object') {
            ...
            for (var _x2 in x_cmds_hashes) {
              if (_x2 in commands_cached_hashes && commands_cached_hashes[_x2] != x_cmds_hashes[_x2]) {
                changed_x_cmds.push(_x2); // search x within x_watches, if x is found, also add that cmd (value) to be cleaned from cache

                if (_x2 in x_watches) {
                  if (!changed_x_cmds.includes(x_watches[_x2])) {
                    changed_x_cmds.push(x_watches[_x2]);
                  }
                }
              ...
              if (watched_vars_cache && _x2 in watched_vars_cache && _x2 in watched_vars) {
                for (var varkey in watched_vars_cache[_x2]) {
                  if (watched_vars[_x2][varkey] != watched_vars_cache[_x2][varkey]) {
                    _this4.x_console.outT({
                      prefix: 'cache,yellow',
                      message: "watched var value by ".concat(_x2, " changed! requesting rebuild of its instances"),
                      color: 'brightYellow'
                    });

                    changed_x_cmds.push(_x2);
                  }
                }
              }
            }
            ...
            if (changed_x_cmds.includes('meta')) {
              ...
              if (meta_cache && typeof meta_cache === 'object' && Object.keys(meta_cache).length > 0) {
                for (var _x3 in meta_cache) {
                  if (_this4.array_intersect(meta_cache[_x3].x_ids.split(','), changed_x_cmds).length > 0) {
                    //remove page 'hashkey' from cache
                    _this4.x_console.outT({
                      prefix: 'cache,yellow',
                      message: "removing ".concat(_x3, " file info from cache .."),
                      color: 'dim'
                    });

                    yield _this4.cache.removeItem(meta_cache[_x3].cachekey);
                    yield _this4.cache.removeItem(meta_cache[_x3].cachekey + '_x_state');
                    amount_cleaned += 1;
                  }
                }
