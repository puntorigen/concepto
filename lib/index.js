    ...
    */
    ...
    addCommands(command_func) {
      ...
      return _asyncToGenerator(function* () {
        ...
        var x_version = '';
        if (_this3.x_commands.meta && _this3.x_commands.meta.version) x_version = _this3.x_commands.meta.version;
        var x_cmds_hashes = {};

        for (var x in _this3.x_commands) {
          if (x == 'meta') {
            x_cmds_hashes[x] = yield _this3.dsl_parser.hash(JSON.stringify(_this3.x_commands[x]));
          } else {
            x_cmds_hashes[x] = yield _this3.dsl_parser.hash(_this3.x_commands[x].func.toString());
          }
        } //console.log('obj_to_hash',x_cmds_hashes);


        var commands_hash = yield _this3.dsl_parser.hash(x_cmds_hashes);
        var commands_cache = yield _this3.cache.getItem('commands_hash');
        var commands_cached_hashes = yield _this3.cache.getItem('commands_hashes');
        if (x_version != '') x_version = 'v' + x_version + ', ';
        var changed_x_cmds = [];

        _this3.x_console.outT({
          message: "x_commands ".concat(x_version, "hash: ").concat(commands_hash),
          color: 'white'
        });
        ...
        if (commands_cache != commands_hash) {
          if (typeof commands_cached_hashes === 'object') {
            ...
            if (changed_x_cmds.includes('meta')) {
              ...
              _this3.x_console.outT({
                message: "x_commands version changed! wiping all cache",
                color: 'brightYellow'
              });

              yield _this3.cache.clear();
            } else {
              if (changed_x_cmds.length > 0) _this3.x_console.outT({
              ...
              var meta_cache = yield _this3.cache.getItem('meta_cache');

              if (meta_cache && typeof meta_cache === 'object' && Object.keys(meta_cache).length > 0) {
                for (var _x2 in meta_cache) {
                  if (_this3.array_intersect(meta_cache[_x2].x_ids.split(','), changed_x_cmds).length > 0) {
                    ...
                    yield _this3.cache.removeItem(meta_cache[_x2].cachekey);
                    yield _this3.cache.removeItem(meta_cache[_x2].cachekey + '_x_state');
                  }
                }
              }
            }
          ...
          yield _this3.cache.setItem('commands_hash', commands_hash);
          yield _this3.cache.setItem('commands_hashes', x_cmds_hashes);
        }
      })();
    }
    /**
    ...
    */
    ...
    process() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var counter_ = 0;
        var meta_cache = {};
        var meta_cache_ = yield _this6.cache.getItem('meta_cache');
        ...
        for (var level2 of x_dsl_nodes) {
          ...
          if (!main) {
            var before_state = JSON.parse(JSON.stringify(_this6.x_state));
            main = yield _this6.process_main(level2, {});
            var state_to_save = obj_diff(before_state, _this6.x_state); //console.log('state_to_save',{ state_to_save });
            ...
            if (main.error && main.error == true) ; else {
              ...
              yield _this6.cache.setItem(level2.hash_content + '_x_state', state_to_save);
            } //console.log('metido al cache:'+level2.id,main);

          } else {
            var cached_state = yield _this6.cache.getItem(level2.hash_content + '_x_state');
            _this6.x_state = deep.all([_this6.x_state, cached_state]);

            if (main.error && main.error == true) {
              yield _this6.cache.removeItem(level2.hash_content); //reprocess the removed failed cached node.

              var _before_state = JSON.parse(JSON.stringify(_this6.x_state));

              main = yield _this6.process_main(level2, {});

              var _state_to_save = obj_diff(_before_state, _this6.x_state); //console.log('state_to_save2',{ state_to_save });
              ...
              if (main.error && main.error == true) ; else {
                ...
                yield _this6.cache.setItem(level2.hash_content, main);
                yield _this6.cache.setItem(level2.hash_content + '_x_state', _state_to_save);
              }
        ...
        yield _this6.onEnd();
        yield _this6.cache.setItem('last_compile_date', new Date()); //add meta cache to cache

        yield _this6.cache.setItem('meta_cache', meta_cache);
        return resp;
      })();
