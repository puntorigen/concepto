  ...
  */
  ...
  class concepto {
    ...
    */
    ...
    autocompleteContentTemplate(record, escapePlaceholders, renderIcon, renderAttrs) {
      return _asyncToGenerator(function* () {
        // param record is an autocomplete object for a given item
        // returns the template to show in an autocomplete view
        var keyword = escapePlaceholders(record.text);
        var hint = escapePlaceholders(record.hint);
        ...
        for (var icon of icons) {
          if (renderIcon) {
            ...
            html += "<img src=\"".concat(icon, ".png\" align=\"left\" hspace=\"5\" vspace=\"5\" valign=\"middle\" />&nbsp;");
          }
        }

        html += "<b>".concat(keyword, "</b><br />");
        html += "".concat(hint, "<br />");
        ...
        */
        ...
        var attributesToHTMLTable = function attributesToHTMLTable() {
          ...
          for (var key in attributes) {
            ...
            html += "<td>".concat(replaceIcons(key), "</td>\n<td>").concat(replaceIcons(escapeSpecial(type_)), "</td>\n<td>").concat(replaceIcons(default_), "</td>\n<td>").concat(replaceIcons(hint), "</td>\n");
            html += "</tr>\n";
          }
        ...
        var generateKeywordXML = /*#__PURE__*/function () {
          var _ref3 = _asyncToGenerator(function* (record) {
            ...
            var xml = "\t\t<keyword type=\"other\" name=\"".concat(record.keyword, "\">\n");
            var html = "<BASE href=\"file://".concat(iconsPath, "/\">\n");
            html += yield _this3.autocompleteContentTemplate(record, replaceIcons, function (icon) {
        ...
        for (var hash_ in _this3.autocomplete.records) {
          ...
          for (var file_ of files) {
            ...
            if (file_ != undefined) {
              var file = path.join(_this3.autocomplete.path, file_ + '.xml'); //this.x_console.outT({ message:`full -> ${file}`, prefix:'autocomplete' });
              //check if file exists and if it contains keyword[name=hash.text]

              var exists = yield _this3.exists(file);
            ...
            if (changed_x_cmds.includes('meta')) {
              ...
              if (meta_cache && typeof meta_cache === 'object' && Object.keys(meta_cache).length > 0) {
                for (var _x7 in meta_cache) {
                  if (_this7.array_intersect(meta_cache[_x7].x_ids.split(','), changed_x_cmds).length > 0) {
                    ...
                    yield _this7.cache.removeItem(meta_cache[_x7].cachekey);
                    yield _this7.cache.removeItem(meta_cache[_x7].cachekey + '_x_state');
                    amount_cleaned += 1;
                  }
                }
